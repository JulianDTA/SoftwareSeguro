...
// ── JwtBearer para la React SPA (SSO) ──────────────────────────────────────
// Valida tokens Keycloak enviados por el cliente React como Bearer.
// En OnTokenValidated mapea el usuario Keycloak a un usuario local de Identity
// para que los controladores API sigan usando ClaimTypes.NameIdentifier local.
.AddJwtBearer(options =>
{
    ...
    options.Events = new JwtBearerEvents
    {
        OnTokenValidated = async ctx =>
        {
            ...
            // Encontrar o crear usuario local por email (SSO auto-provisionamiento)
            ...
            // Reemplazar el sub de Keycloak con el ID local de Identity
            var existingId = identity.FindFirst(ClaimTypes.NameIdentifier);
            if (existingId != null) identity.RemoveClaim(existingId);
            identity.AddClaim(new Claim(ClaimTypes.NameIdentifier, user.Id));

            // Agregar roles locales de la BD
            ...
            // Sincronizar roles de Keycloak (resource_access.refugiohuellas.roles)
            ...
            if (resourceAccessClaim != null)
            {
                try
                {
                    ...
                    if (doc.RootElement.TryGetProperty("refugiohuellas", out var clientRoles) &&
                        clientRoles.TryGetProperty("roles", out var roles))
                    {
                        foreach (var r in roles.EnumerateArray())
                        {
                            ...
                            {
                                var normalized = char.ToUpper(roleName[0]) + roleName[1..];
...
var dpKeysPath = Environment.GetEnvironmentVariable("DPKEYS_PATH")
    ?? (builder.Environment.IsDevelopment()
        ? Path.Combine(Path.GetTempPath(), "refugiohuellas-dpkeys")
        : "/var/data/dpkeys");
